rm(list=ls(all=T))
setwd("E:/Edwisor/Notes")

#Load Libraries
x = c("ggplot2", "corrgram", "DMwR", "caret", "randomForest", "unbalanced", "C50", "dummies", "e1071", "Information",
     "MASS", "rpart", "gbm", "ROSE", 'sampling', 'DataCombine', 'inTrees')
y = c("inTrees", "DataCombine", "sampling","gbm","Information","e1071","dummies","C50","unbalanced","caret","DMwR","corrgram")
install.packages(x)
lapply(x, require, character.only = TRUE)
rm(y)

## Read the data
marketing_train = read.csv("m_tr.csv", header = T, na.strings = c(" ", "", "NA"))
...
marketing_train$month = as.factor(as.character(maketing_train$month))
